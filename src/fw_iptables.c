...
iptables_fw_init(void)
{
	...
	/* CHAIN_OUTGOING, packets for tcp port 443, redirect to gw_port + 1 on primary address for the iface */
	rc |= iptables_do_command("-t nat -A " CHAIN_OUTGOING " -p tcp --dport 443 -j DNAT --to-destination %s:%d", gw_address, gw_port + 1);
